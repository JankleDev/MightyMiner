...
import com.jelly.MightyMiner.features.impl.PlayerESP;
...
public class DrawUtils {
    ...
    // From Farmhelper these honestly just look better :D
    ...
    public static void drawPoint(Vec3 vec, Color color) {
        ...
        double a = .05;
        drawBox(new AxisAlignedBB(vec.xCoord-a, vec.yCoord-a, vec.zCoord-a,
            vec.xCoord+a, vec.yCoord+a, vec.zCoord+a).offset(-d0, -d1, -d2), color);
    }

    public static void drawEntity(Entity entity, Color color){
        Vec3 posVecBottom = entity.getPositionVector().addVector(-.5, 0, -.5);
        Vec3 posVecTop = entity.getPositionVector().addVector(.5, entity.height + .3, .5);
        double d0 = Minecraft.getMinecraft().getRenderManager().viewerPosX;
        double d1 = Minecraft.getMinecraft().getRenderManager().viewerPosY;
        double d2 = Minecraft.getMinecraft().getRenderManager().viewerPosZ;
        AxisAlignedBB entityAABB = new AxisAlignedBB(
            posVecBottom.xCoord, posVecBottom.yCoord, posVecBottom.zCoord,
            posVecTop.xCoord, posVecTop.yCoord, posVecTop.zCoord
        ).offset(-d0, -d1, -d2);
        drawBox(entityAABB, color);
    }
    ...
    public static void drawLine(Vec3 from, Vec3 to, Color color) {
        GlStateManager.pushMatrix();
        GlStateManager.enableBlend();
        GlStateManager.disableDepth();
        GlStateManager.disableLighting();
    ...
    public static void drawLine(Vec3 to, Color color) {
        drawLine(new Vec3(0, Minecraft.getMinecraft().thePlayer.getEyeHeight(), 0), to, color);
    }
}
